...
begin
  ...
  FDS.position:=SizeOf(TS386Shapka){+SizeOf(TPrimech)};
  DataSingle := nil;
  ...
  for j := 0 to SignalShapka.NchanalOriginal - 1 do
    begin
      for i:=0 to (SignalShapka.Npoints-1) do
        begin
         FDS.Position:=SizeOf(TS386Shapka)+i*(4*SignalShapka.NchanalOriginal)+j*4;
...
//    DateEksp:=trunc(dateEksp*86400.0);
...
//   SignalShapka.date_eksp:=dateEksp-Signalshapka.TD*jmax/86400.0;
...
//    FDD.Destroy;
    ...
    ChanArray[j].HEADER:='S386_'+inttostr(j);
   ...
   FDD := TFileStream.Create(DestFileName + '_ch' + inttostr(j + 1) + '.gms', fmCreate);
    for i:=0 to (SignalShapka.Npoints-1) do
      begin
       FDS.Position:=SizeOf(TS386Shapka)+i*(4*SignalShapka.NchanalOriginal)+j*4;
       FDS.Read(dataS386[i,j],4) ;
       DataSingle[i]:=DataS386[i,j]*1/429497 ;
     end;
...
try
 ...
 try
  ...
  FDS.Read(S386, SizeOf(TS386Shapka));
   ...
   SignalShapka.date_eksp:= encodedatetime( S386.yyb+2000,S386.mmmb,S386.ddb,
   ...
   SignalShapka.NChanal:=1;//5;//0;
   for i:=1 to 4 do
    if S386.g[i]<>0 then SignalShapka.NChanal := SignalShapka.NChanal +1;
   SignalShapka.NPoints:=(File_size-SizeOf(S386)) div 4 div SignalShapka.NChanal ;

   SignalShapka.NChanalOriginal:=SignalShapka.NChanal; //5l
   ...
   SignalShapka.typeofsignal:='S386';
   SignalShapka.e_functime:='t';
   SignalShapka.TipData := cRealLong{S386.TipData};
//   SignalShapka.TD := 1 / S386.frq;
   SignalShapka.NChanalOriginal := SignalShapka.NChanal ;
  case S386.frq of
    0:begin
       SignalShapka.TD:=1/25;//(Finish-Start)*24*60*60/DataProperties.NPoints ;
//       ext:='25';
      end;
    1:begin
       SignalShapka.TD:=1/50;
//       ext:='50';
      end;
    2:begin
       SignalShapka.TD:=1/100;
//       ext:='100';
      end;
    3:begin
       SignalShapka.TD:=1/200;
//       ext:='200';
      end;
    4:begin
       SignalShapka.TD:=1/128;
//       ext:='128';
      end;
   end;
...
exports
  GETINFO_S386 name 'GETINFO_S386',
  GETDATA_S386 name 'GETDATA_S386';
end.
