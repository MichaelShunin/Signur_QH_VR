...
begin
 ...
 dm.GetStringsObjectsFromQuery(dm.sql, device_CB.Properties.Items,'select device_Name, id_Device from devices where not device_type is null order by DEVICE_name ', 'device_name', 'ID_Device');
// Importclick(nil);
end;
...
begin
  inherited;
   if trim(device_number.text)='' then
    begin
     ...
     exit;
      ...
      if trim(cxFTP_PASSWORD.text)='' then
    begin
     showmessage('Вы не задали код интернет-доступа к архиву прибора');
     cxFTP_PASSWORD.text:='0';
     modalresult:=mrNone;
     exit;
    end;
   ...
   if trim(Device_CB.text)='' then
    ...
    id_dev:=integer(Device_CB.Properties.Items.objects[device_CB.itemindex]);

    if (id_dev in [15]) then   // АКРОН -01
     begin
        if  strtoint(trim(Device_Number.text))<4000 then
      begin
        showmessage('Для прибора АКРОН-01 с заводским номером менее 4000 допустим только интерфейс RS232');
        modalresult:=mrNone;
       exit;
      end;
     end ;

   if ((rgAccess.itemindex=1) and (ModBus_address.value > 127) or (ModBus_address.value <1)) then
     begin
      showmessage('Ошибка ввода адреса ModBus. Адреc должен быть в пределах от 1 до 127');
      modalresult:=mrNone;
      exit;
     end;

    if not (id_dev in [13,14,15,16,22,23]) then
    begin

   dm.q.sql.clear;
   dm.q.sql.add('select d.startnumber_largearch from  devices d where d.id_device = :id_device ');
   dm.q.parambyname('id_device').asinteger:= integer(Device_CB.Properties.Items.objects[device_CB.itemindex]);
   dm.q.Open;
   if (DM.q.FieldByName('startnumber_largearch').AsInteger=4000) then
    begin
     if  strtoint(trim(Device_Number.text))<4000 then
      begin
        showmessage('Для прибора допустим заводской номер более 4000');
        modalresult:=mrNone;
       exit;
      end;
   end;
 ...
 try
   ...
   dm.CREATE_EKSP_AND_DEVICEPARAM.ParamByName('modBus').asinteger:=integer(rgAccess.Itemindex=1);
   ...
   dm.CREATE_EKSP_AND_DEVICEPARAM.ParamByName('modem').asinteger:=integer(cbVia_Modem.checked);
  dm.CREATE_EKSP_AND_DEVICEPARAM.ParamByName('Tone_Pulse').asinteger:=Tone_Pulse.ItemIndex;
   ...
   if rgAccess.ItemIndex =3 then
    ...
    dm.q.sql.clear;
    dm.q.sql.add('update eksp set FTP_PASSWORD = :FTP_PASSWORD  where eksp_nomer = :eksp_nomer ');
    dm.q.parambyname('eksp_nomer').asinteger:=id;
    dm.q.parambyname('FTP_PASSWORD').asinteger:=strtoint(trim(cxFTP_PASSWORD.text));
    dm.q.ExecSQL;

    if rgAccess.ItemIndex =2 then
    begin
     dm.q.sql.clear;
     dm.q.sql.add('update eksp set interface_mode = 2 where eksp_nomer = :eksp_nomer ');
     dm.q.parambyname('eksp_nomer').asinteger:=id;
     dm.q.ExecSQL;
    end;
...
begin
 ...
 TabsheetModbus.TabVisible:=rgAccess.itemindex=1;
// TabsheetBlueTooth.TabVisible:=rgAccess.itemindex=3;
 case rgAccess.itemindex of
  0: if cbVia_modem.checked then Pagecontrol1.ActivePage:=TabsheetModem else Pagecontrol1.ActivePage:=TabsheetMemo;
  1: Pagecontrol1.ActivePage:=TabsheetModbus;
  2: begin Pagecontrol1.ActivePage:=TabsheetMemo; cbVia_modem.checked:=False;end;
  //3: Pagecontrol1.ActivePage:=TabsheetBlueTooth;
 end;
end;

procedure TAddSignurDeviceForm.cbVia_modemClick(Sender: TObject);
begin
  inherited;
  TabsheetModem.TabVisible:=cbVia_modem.checked;
  if rgAccess.ItemIndex=2 then cbVia_modem.checked:=False;
 if  cbVia_modem.checked  then
   begin
    Pagecontrol1.ActivePage:=TabsheetModem;
   end
  else
   begin
   TabsheetModem.TabVisible:=cbVia_modem.checked;
   ...
   // TabsheetBlueTooth.TabVisible:=rgAccess.itemindex=3;
   case rgAccess.itemindex of
    0,2: Pagecontrol1.ActivePage:=TabsheetMemo;
    1: Pagecontrol1.ActivePage:=TabsheetModbus;
