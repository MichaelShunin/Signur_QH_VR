...
type
  TDevice_Form = class(TForm)
    ...
    procedure cbbTypeDeviceCloseUp(Sender: TObject);
    ...
    procedure edDeviceNumberKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure cbCustomerKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure cbCustomerCloseUp(Sender: TObject; Accept: Boolean);
...
{  MyDecimal:=StrAlloc(10);
 ...
 dm.GetName_IDStringsFromQuery(dm.q,'select name, ID from devicetypes where (not (Id in (5,8,9))) order by id ',cbbTypeDevice.Items,'name', 'ID');
 if cbbTypeDevice.items.count>0 then cbbTypeDevice.Itemindex:=0;
...
var
 i:integer;
begin
 cbbTypeDevice.ItemIndex:=cbbTypeDevice.Items.IndexOfObject(Tobject(q_devices.fieldbyname('TypeDevice_ID').asinteger));
 ...
 case rgRG_Filter.itemindex of
  1:begin
    ...
    btnTypeDevice.visible:=false;  cbbTypeDevice.visible:=false;
    end;
  0: begin
      for i:=0 to GridFrame.Grid.columns.count-1 do
       if (GridFrame.grid.Columns[i].FieldName='DEVICETYPENAME') then
        GridFrame.grid.Columns[i].visible:=true;
          btnTypeDevice.visible:=true;  cbbTypeDevice.visible:=true;
    end;
  end;
end;
...
procedure TDevice_Form.cbbTypeDeviceCloseUp(Sender: TObject);
begin
 q_devices.edit;
 q_devices.fieldbyname('DeviceTypeName').asstring:=  cbbTypeDevice.text;
 q_devices.post;
end;
...
//      HTML.Add('<BR>');
      HTML.Add('Тип водовода: ' + cbbTypeDevice.text+'<BR>');
...
while not q_QH.eof do
 ...
 if q_devices.fieldbyname('CalcMode').asstring='USERTABLE' then q_QH.FieldByName('X_Value').AsFloat:=0
  ...
  if x=HMax then q_QH.FieldByName('X_Value').AsFloat:=x;
  q_QH.post;
  q_qh.Next;
  i:=i+1;
 end;
...
begin
 ...
 tsDeviceAttribute.TabVisible:=False;
...
begin
 ...
 Qmax:=Y_Array[0];
 for i:=1 to N6-1 do
   if Qmax<Y_array[i] then Qmax:=Y_array[i];
...
//    xlreportparam.value :=inttostr(p);
  ...
  xlreportparam.value :='0000';  // ТОЛЬКО ДЛЯ SIGNUR_QH
  
  if not DirectoryExists(extractfiledir(Application.exename)+'\XLS_TMP\') then
       CreateDir(extractfiledir(Application.exename)+'\XLS_TMP\')  ;

    FName:=extractfiledir(Application.exename)+'\XLS_TMP\'+q_devices.fieldbyname('name').asstring+'_'+
              q_devices.fieldbyname('deviceNumber').asstring+'_'+ FormatDateTime('ddmmyy_hhmmss',Now)+
              dm.excel_ext;
             //'.xls';
     deletefile( extractfiledir(Application.exename)+'\XLS_TMP\'+q_devices.fieldbyname('name').asstring+'_'+
              q_devices.fieldbyname('deviceNumber').asstring+'_'+ FormatDateTime('ddmmyy_hhmmss',Now)+'.xlsx');
     deletefile( extractfiledir(Application.exename)+'\XLS_TMP\'+q_devices.fieldbyname('name').asstring+'_'+
              q_devices.fieldbyname('deviceNumber').asstring+'_'+ FormatDateTime('ddmmyy_hhmmss',Now)+'.xls');
...
procedure TDevice_Form.N10Click(Sender: TObject);
var Id:integer;
begin
...
begin
 id_device := q_devices.fieldbyname('id').asinteger;
  ...
  try
    GetValueFromQueryText_ID('select ID, Name from DeviceTypes where (not (Id in (5,8,9))) order by sortorder', 'Name', 'ID');
...
procedure TDevice_Form.cbCustomerKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if Key = VK_RETURN then rg_rs.setfocus;
end;

procedure TDevice_Form.cbCustomerCloseUp(Sender: TObject; Accept: Boolean);
begin
  focuscontrol(rg_RS);
...
procedure TDevice_Form.cbCustomerClick(Sender: TObject);
begin
//if cbConverter.ItemIndex  = -1 then exit;
 if (q_devices.State in [dsedit]) then
     ...
     {*  CurrentPoint:=trunc(tmpX*Signal.fieldbyname('FClock').asfloat);
      CurrentMousePos:=x;
      if ((CurrentPoint>=0) and (CurrentPoint<Yvalues.count-1)) then
      begin
{       MousePos.Caption:=TChart(sender).leftAxis.title.Caption+'='+
                         Formatfloat(ValuesFormatY,YValues.Value[CurrentPoint])+
                         '    Y='+FormatFloat(ValuesFormatY,TmpY)+//GetvertAxis.LabelValue(tmpY)+
                         '    X='+FormatFloat(FormatX,TmpX);//GetHorizAxis.LabelValue(tmpX);
 }      MousePos.Caption:='t,c ='+FormatFloat('0.00000',TmpX);//GetHorizAxis.LabelValue(tmpX);

       CurrentXStr:= 'X='+FormatFloat(FormatX,TmpX);
       CurrentX:=TmpX;
       CurrentY:=YValues.Value[CurrentPoint];
       if oborotpanel.Visible then
        begin
{         oborotpanel.Caption:=formatfloat('0',Charr[4].Series[0].YValues.value[CurrentPoint])+' градусов на '+
                              formatfloat('0',Charr[5].Series[0].YValues.value[CurrentPoint])+' обороте ';
         oborotpanel.hint:= formatfloat('0',Charr[4].Series[0].YValues.value[CurrentPoint])+' град.на '+
                              formatfloat('0',Charr[5].Series[0].YValues.value[CurrentPoint])+' об. ';
}         oborotpanel.Caption:='  '+EtapInfo+'  Число оборотов: '+formatfloat('0',Charr[5].Series[0].YValues.value[CurrentPoint])+
                                         ' целых + '+formatfloat('0',Charr[4].Series[0].YValues.value[CurrentPoint])+' град.';
//    + inttostr(CurrentPoint);
         oborotpanel.hint:= EtapInfo_for_param+' '+formatfloat('0',Charr[5].Series[0].YValues.value[CurrentPoint])+' об.+'+formatfloat('0',Charr[4].Series[0].YValues.value[CurrentPoint])+' град.';     end;
      end
                      else
                       begin
                        MousePos.Caption:='';
                        OldActiveChart:=-1;ActiveChart:=-1;
                        TChart(Sender).Color:=clcream;
                       end;
    *}
