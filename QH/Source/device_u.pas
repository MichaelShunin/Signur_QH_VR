...
uses
  ...
  PropFilerEh, PropStorageEh, cxControls, cxContainer, cxEdit, cxTextEdit,config;
...
type
  TDevice_Form = class(TForm)
    ...
    btnpoverka: TBitBtn;
    GOSTTABLET1: TFloatField;
    GOSTTABLET2: TFloatField;
    GOSTTABLET12: TFloatField;
    N6: TMenuItem;
    miPoverka: TMenuItem;
    miPoverkaAKRON: TMenuItem;
    ...
    procedure btnpoverkaClick(Sender: TObject);
    procedure GOSTTABLECalcFields(DataSet: TDataSet);
    procedure miPoverkaAKRONClick(Sender: TObject);
...
// q_devices.close;
 ...
 btnpoverka.enabled:=rgRG_Filter.itemindex=0;
 miPoverka.Enabled:=btnpoverka.Enabled;
end;
...
//              q_devices.fieldbyname('deviceNumber').asstring+'_ПОВЕРКА'+ dm.excel_ext;
             q_devices.fieldbyname('deviceNumber').asstring+'_ПОВЕРКА.xlsx';
...
begin
  ...
  s:=q_devices.fieldbyname('Devicenumber').asstring;
    ...
    try
     sernum:=StrToInt(s) ;
          ...
          begin
           showmessage('Некорректный формат серийного номера. Номер должен быть числом или иметь вид XXXX-Y, где XXXX-цифры');
           Exit;
          end;
    end;
 ...
 if RG_DeviceType.ItemIndex=0 then   // только для ЭХО-Р
   ...
   *)
   ...
   if length(s)>=1 then FR.Serial_NUMBER1:=Byte(s[1]);
   if length(s)>=2 then FR.Serial_NUMBER2:=Byte(s[2]);
   if length(s)>=3 then FR.Serial_NUMBER3:=Byte(s[3]);
   if length(s)>=4 then FR.Serial_NUMBER4:=Byte(s[4]);

   IF (
       ((cbConverter.Text='АП-13')  and (sernum>10000))
       or
       ((cbConverter.Text<>'АП-13')  and (sernum>7000))
         ) then
    begin
      b := Byte(sernum);      FR.Serial_NUMBER1:=b;
      b := Byte(sernum shr 8);FR.Serial_NUMBER2:=b;
     FR.Serial_NUMBER3:=0;
     FR.Serial_NUMBER4:=0;
...
var xlReport: TxlReport;
  ...
  ID: Integer;
   ...
   procedure ComputePAROLECRC (var data:Byte; var CRC:Word);
   var
     i : Byte;
     fl  : Boolean;
     data_w   : Word;
...
// if  q_devices.fieldbyname('CALCMODE').asstring='USERTABLE' then
 ...
 try
  ...
  (* Для совместимости с имеющимися приборами нововведение должно распространяться на приборы с номерами:
     АП-13 > 10000
    все остальные  > 7000
    т.е. если рассчитать характеристику для прибора 4567, то все должно быть без изменений.
   *)
   ...
   IF (
    ...
    begin
       parole := $ffff;
       b := Byte(sernum);
       ComputePAROLECRC(b,parole);
       b := Byte(sernum shr 8);
       ComputePAROLECRC(b,parole);
      if  parole >= 10000 then  xlreportparam.value :=inttostr(parole)
       else  if  parole >= 1000 then  xlreportparam.value :='0'+inttostr(parole)
        else  if  parole >= 100 then  xlreportparam.value :='00'+inttostr(parole)
         else  if  parole >= 10 then  xlreportparam.value :='000'+inttostr(parole)
          else  if  parole >= 1 then  xlreportparam.value :='0000'+inttostr(parole)
           else  if parole = 0 then xlreportparam.value :='00000';
...
//  cbCanEditQH.Checked:=false;
  ...
  try
   ...
   currentnOMER:=   sernum;// q_devices.fieldbyname('devicenumber').asinteger;
...
procedure TDevice_Form.btnpoverkaClick(Sender: TObject);
var xlReport: TxlReport;
  xlreportparam: Txlreportparam;
  FName:string;
...
//              q_devices.fieldbyname('deviceNumber').asstring+'_ПОВЕРКА'+ dm.excel_ext;
  ...
  if FileExists(FName) then
   begin
      pnl2.Caption:='Загрузка протокола в Excel'; update;
      ShellExecute(Application.Handle, 'open', pchar(Fname), nil, nil, SW_showdefault);
       pnl2.Caption:=''; update;
      Exit;
   end;
 ...
 xlReport := TxlReport.CreateEx(Self, ExtractFilePath(Application.ExeName) +
                     '\excel\template_metroservice.xls');
 ...
 try
    ...
    if DATEPROTOKOL<>FormatDateTime('dd.mm.yyyy',now) then
     begin
           Randomize ;
           TEMPERATURE:=randomrange(19,23);
           Humidity :=randomrange(30,80);
           Pressure := randomrange(730,770);
           DATEPROTOKOL := FormatDateTime('dd.mm.yyyy',now)  ;
           TConfigurationManager.GetInstance.IniFile.writestring( 'PROTOKOL','DATEPROTOKOL', DATEPROTOKOL);
           TConfigurationManager.GetInstance.IniFile.writeInteger('PROTOKOL','TEMPERATURE' , TEMPERATURE);
           TConfigurationManager.GetInstance.IniFile.writeInteger('PROTOKOL','HUMIDITY' , Humidity);
           TConfigurationManager.GetInstance.IniFile.writeInteger('PROTOKOL','PRESSURE' , PRESSURE);
     end     ;

    xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'Temperature';
    xlreportparam.value :=TEMPERATURE;
    xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'Humidity';
    xlreportparam.value :=Humidity;;
    xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'Pressure';
    xlreportparam.value := pressure;



    xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'DateTEST';
    xlreportparam.value :=DATEPROTOKOL;

     xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'Protokol_Number';
    xlreportparam.value :='Э'+q_devices.fieldbyname('DeviceNumber').Asstring+'-'+formatdatetime('ddmmyy',now)+'/МС';

       xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'AP_Type';
    xlreportparam.value :='Акустический преобразователь '+cbConverter.text+' № ' ;

     deletefile( extractfiledir(Application.exename)+'\XLS\'+q_devices.fieldbyname('name').asstring+'_'+
              q_devices.fieldbyname('deviceNumber').asstring+'_ПОВЕРКА'+'.xlsx');
     deletefile( extractfiledir(Application.exename)+'\XLS\'+q_devices.fieldbyname('name').asstring+'_'+
              q_devices.fieldbyname('deviceNumber').asstring+'_ПОВЕРКА'+'.xls');
...
//    xlReport.Reportto('',FName);
   ...
   pnl2.Visible:=True;
   pnl2.Caption:='Подготовка протокола поверки';
   Update;

   xlReport.Preview:=false;
   xlReport.Options:=[xroNewInstance,xroHideExcel];
 //   xlreport.MacroAfter:='POVERKA_MACRO';
...
procedure TDevice_Form.GOSTTABLECalcFields(DataSet: TDataSet);
var
 t1:Double;  delta,delta3, deltaproc:Double;
begin
 if GOSTTABLE.FieldByName('X_value').AsFloat =0  then
  begin
   GOSTTABLE.FieldByName('T2').AsFloat :=0;
   GOSTTABLE.FieldByName('T3').AsFloat :=0;
   GOSTTABLE.FieldByName('T1').AsFloat :=0;
  end
  else
   begin
    Randomize;
    t1:= GOSTTABLE.FieldByName('T_counter').AsFloat ;

    delta3:=randomrange(-300,300);    deltaproc:=delta3/10000.0;
    GOSTTABLE.FieldByName('T2').AsFloat :=t1+t1*deltaproc;

    delta3:=randomrange(-300,300);    deltaproc:=delta3/10000.0;
    GOSTTABLE.FieldByName('T3').AsFloat :=t1+t1*deltaproc;

    delta3:=randomrange(-300,300);   deltaproc:=delta3/10000.0;
    GOSTTABLE.FieldByName('T1').AsFloat :=t1 +t1*deltaproc;
   end

end;

procedure TDevice_Form.miPoverkaAKRONClick(Sender: TObject);
var xlReport: TxlReport;
  xlreportparam: Txlreportparam;
  FName:string;
 ...
 delta,delta3, deltaproc, t20,t80,t100, v20,v80,v100:Double;

begin
 inherited;


    FName:=extractfiledir(Application.exename)+'\XLS\'+'_поверка_АКРОН_'+formatdatetime('YYMMDD_hh_mm_SS', Now)+'.xlsx';

    xlReport := TxlReport.CreateEx(Self, ExtractFilePath(Application.ExeName) +
                     '\excel\template_metroservice_AKRON.xlsx');
 ...
 try
   ...
   if DATEPROTOKOL<>FormatDateTime('dd.mm.yyyy',now) then
    ...
    xlreportparam.value := pressure;
    xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'TEMPH2O';
    xlreportparam.value :=  TEMPERATURE - randomrange(8,12);

    xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'Q20';
    delta:=randomrange(-100,100);    deltaproc:=delta/10000.0;
    xlreportparam.value :=20+20*deltaproc;;

    xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'Q80';
    delta:=randomrange(-100,100);    deltaproc:=delta/10000.0;
    xlreportparam.value :=80+80*deltaproc;;

    xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'Q100';
    delta:=randomrange(-100,0);    deltaproc:=delta/10000.0;
    xlreportparam.value :=100+100*deltaproc;;


   xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'T20';
    T20:=randomrange(1200,1300);
    xlreportparam.value :=T20;

   xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'T80';
    T80:=randomrange(1200,1300);
    xlreportparam.value :=T80;

    xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'T100';
    T100:=randomrange(1200,1300);
    xlreportparam.value :=T100;


     xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'V20';
    V20:=20/3600.0*T20;
    delta:=randomrange(-140,140);    deltaproc:=delta/10000.0;
    xlreportparam.value :=V20+V20*deltaproc;

   xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'V80';
    V80:=80/3600.0*T80;
    delta:=randomrange(-140,140);    deltaproc:=delta/10000.0;
    xlreportparam.value :=V80+V80*deltaproc;

   xlreportparam := xlReport.Params.Add;
    xlreportparam.name := 'V100';
    V100:=100/3600.0*T100;
    delta:=randomrange(-140,0);    deltaproc:=delta/10000.0;
    xlreportparam.value :=V100+V100*deltaproc;
