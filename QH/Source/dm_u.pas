...
type
  ...
  TDM = class(TDataModule)
    ...
    CREATENEWDEVICE1: TIBStoredProc;
    ...
    CREATENEWDEVICE_NEW: TIBStoredProc;
    AnyQuery: TIBQuery;
    ...
    QS: TIBSQL;
    q_Check_Field_LO: TIBQuery;
    ibstrngfld_Check_Field_LORDBFIELD_NAME: TIBStringField;
    CHANGEDEVICETYPE_TO_USERTABLE: TIBStoredProc;
...
procedure TDM.checkUPdate_DB;
var i:integer;
begin
 //10.07.2014
  q_Check_Field_LO.close;
  q_Check_Field_LO.Open;
  if (q_Check_Field_LO.RecordCount<1)
   then
   begin
     q_Check_Field_LO.Close; 
     for i:=1 to 4 do
      begin
       QS.SQL.Clear;
       QS.sql.loadfromfile(ExtractFilePath(Application.exename)+'\SQL\'+inttostr(i)+'.sql');
       qs.ExecQuery;           ;
...
//   db.DatabaseName := extractfilepath(application.exename)+'db\QH.gdb';//ReadString(DB_SECTION, 'DB', '');
   db.DatabaseName := changesymbol('127.0.0.1:'+extractfilepath(application.exename)+'db\QH.gdb','\','/');
   Import_Root:=   ReadString('Import','Root' , GetCurrentDir);
   DATEPROTOKOL:=   Readstring('PROTOKOL','DATEPROTOKOL' , '01.01.2000');
   TEMPERATURE:=   ReadInteger('PROTOKOL','TEMPERATURE' , 20);
   Humidity:=   ReadInteger('PROTOKOL','HUMIDITY' , 65);
   PRESSURE:=   ReadInteger('PROTOKOL','PRESSURE' , 740);
...
if not DB.connected then
  begin
   ...
   Settings.Close;
   Settings.Open;
 {   if Settings.locate('VALUE_NAME','MAXLINE_INPUT',[]) then
      MAXLINE_INPUT:= Settings.fieldbyname('Value_Str').value;
    if Settings.locate('VALUE_NAME','MAXCOL_INPUT',[]) then
      MAXCOL_INPUT:= Settings.fieldbyname('Value_Str').value;
    if Settings.locate('VALUE_NAME','INPUTFILE',[]) then
      INPUTFileName:= Settings.fieldbyname('Value_Str').value;
    if Settings.locate('VALUE_NAME','OUTPUTFILE',[]) then
      OUTPUTFileName:= Settings.fieldbyname('Value_Str').value;
    if Settings.locate('VALUE_NAME','FORTRANEXENAME',[]) then
      FORTRANEXENAME:= Settings.fieldbyname('Value_Str').value;
  }  Settings.Close;
  end;
